...
public class TimeSheetReport {
	...
	public JPanel generateReportPanel() {
		...
		String title = "Time Sheet Report (" + new SimpleDateFormat("EEE, MMM dd, yyyy").format(startDate) + " - "
				+ new SimpleDateFormat("EEE, MMM dd, yyyy").format(endDate) + ")";
	...
	private JTable generateReportTable() {
		...
		String[] columnNames = { "Employee Name", "Shift Start Date", "Shift End Date", "Shift Start Time",
				"Shift End Time", "Hours Worked", "Overtime Comments" };
		...
		for (Employee emp : employees) {
			...
			if (empTimeSheets != null) {
				for (TimeSheet ts : empTimeSheets) {
					...
					model.addRow(new Object[] { emp.getName(), sdf.format(ts.getShiftStartDate()),
							sdf.format(ts.getShiftEndDate()), ts.getShiftStartTime(), ts.getShiftEndTime(),
							ts.getFormattedHoursWorked(), ts.getOvertimeComment() });
				}
			}
		}
